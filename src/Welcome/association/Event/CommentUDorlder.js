...
function FormDialog(props) {
  ...
  const handleClose = () => {
    /* on remet playDialog a false quand on ferme le Dialog */
    setOpen(false);
    setPlayDialog(false);
  };
  ...
  const handleSubmit = (e) => {
    ...
    axios
      ...
      .then((res) => {
        ...
        setPlayDialog(
          false
        ); /* on remet a false le playDialog lorsque on a envoyé car on veut un changement de valeur pour réexecuté le useFect */

        setComment(
          res.data
        ); /* on s'assure a chaque fois qu'on envoie on stock dans le state coment le commentaire ; a cause des problèmes que j'ai eu j'ai fait cette solution */
      })
      .catch((error) => {
...
export default function CommentUDolder(props) {
  ...
  return (
    <div>
      ...
      >
        ...
        >
          {" "}
          {/* chaque clic je veux afficher le Dialog donc true */}
